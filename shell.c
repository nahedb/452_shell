...
int display_prompt(){
    //This is the display prompt to the user
    //that is before every command. 
    char hostname[1024];
...
int read_commands(char* cmd, char** token){
    ...
    char * tempcmd;
    tempcmd = strtok(cmd, " ");
    //This operation tokenizes the command and 
    //stores each token initially into tempcmd
    //and then into the array of token at count.
    while(tempcmd != NULL){
        token[count] = tempcmd;
        tempcmd = strtok(NULL, " ");
        count++;
    }
    //Adds a NULL statement in the last place in
    //the Char array of token. This allows execvp
    //to stop at NULL.
    token[count] = NULL;
    return 0;
}
